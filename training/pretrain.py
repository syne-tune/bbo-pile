...
def fit(
    ...
    if eval.initial_validation:
        val_loss = validate(fabric, model, val_dataloader, max_iters=eval.max_iters, loss_weights=loss_weights)
        val_loss = f"{val_loss:.3f}"
    else:
        fabric.print("Verifying settings ...")
        validate(fabric, model, val_dataloader, max_iters=2, verbose=False, loss_weights=loss_weights)  # sanity check
        val_loss = "n/a"
